...
class Body:
    ...
    def setup_vars_for_simulation(self, num):
        self.axis, self.ecc, self.inc, self.Omega, self.omega, self.M, self.longitude, self.varpi, self.X, self.Y, self.Z, self.Vx, self.Vy, self.Vz= (
            np.zeros(num),
            np.zeros(num),
            np.zeros(num),
            np.zeros(num),
        ...
        for mmr in self.mmrs:
            self.planetpos1[mmr.to_s()] = np.zeros([num,6])
            self.planetpos2[mmr.to_s()] = np.zeros([num,6])

        for secular in self.secular_resonances:
            self.planetpos1[secular.to_s()] = np.zeros([num,6])
            self.planetpos2[secular.to_s()] = np.zeros([num,6])

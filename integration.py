...
class IntegrationEngine:
    ...
    def run_integration(self, bodies: List[resonances.Body], times, progress=False):
        ...
        for i, time in iterations:
            ...
            # Update body data
            for body in bodies:
                self._update_body_data(body,ps, os, i)

    def _update_body_data(self, body: resonances.Body, particles, orbits, time_index):
        """Update body orbital data and calculate resonant angles."""

        particle = particles[body.index_in_simulation]
        body.X[time_index] = particle.x
        body.Y[time_index] = particle.y
        body.Z[time_index] = particle.z
        body.Vx[time_index] = particle.vx
        body.Vy[time_index] = particle.vy
        body.Vz[time_index] = particle.vz
        ...
        for mmr in body.mmrs:
            planetposs = [particles[idx] for idx in mmr.index_of_planets]
            planets = [orbits[idx - 1] for idx in mmr.index_of_planets]
            body.angle(mmr)[time_index] = mmr.calc_angle(orbit, planets)

            #first planet (2-body)
            body.planetpos1[mmr.to_s()][time_index] = np.stack([planetposs[0].x,planetposs[0].y,planetposs[0].z,planetposs[0].vx,planetposs[0].vy,planetposs[0].vz])

            #second planet (3-body)
            if resonances.MMR.number_of_bodies(mmr)==3:
                body.planetpos2[mmr.to_s()][time_index] = np.stack([planetposs[1].x,planetposs[1].y,planetposs[1].z,planetposs[1].vx,planetposs[1].vy,planetposs[1].vz])
        ...
        for secular in body.secular_resonances:
            ...
            planetposs = [particles[idx - 1] for idx in secular.index_of_planets]
            body.angle(secular)[time_index] = secular.calc_angle(orbit, planets)

            #first planet (2-body)
            body.planetpos1[secular.to_s()][time_index] = np.stack([planetposs[0].x,planetposs[0].y,planetposs[0].z,planetposs[0].vx,planetposs[0].vy,planetposs[0].vz])

            #second planet (3-body)
            if len(planets) >=1:
                body.planetpos2[secular.to_s()][time_index] = np.stack([planetposs[1].x,planetposs[1].y,planetposs[1].z,planetposs[1].vx,planetposs[1].vy,planetposs[1].vz])
